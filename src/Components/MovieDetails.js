...
class MovieDetails extends Component {
  ...
  rateClicked = (stars) => (evt) => {
    fetch(
      `${process.env.REACT_APP_API_URL}/api/movie/${this.props.movie.id}/rate_movie/`,
      {
        method: "POST",
        headers: {
        ...
        {mov ? (
          <div className="movieDetails__Info">
            ...
            <div className="movieDetails__Description">
              <FontAwesomeIcon
                icon="star"
                className={mov.avg_rating > 0 ? "movieDetails__Star" : ""}
              />
              <FontAwesomeIcon
                icon="star"
                className={mov.avg_rating > 1 ? "movieDetails__Star" : ""}
              />
              <FontAwesomeIcon
                icon="star"
                className={mov.avg_rating > 2 ? "movieDetails__Star" : ""}
              />
              <FontAwesomeIcon
                icon="star"
                className={mov.avg_rating > 3 ? "movieDetails__Star" : ""}
              />
              <FontAwesomeIcon
                icon="star"
                className={mov.avg_rating > 4 ? "movieDetails__Star" : ""}
              />
